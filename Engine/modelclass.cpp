...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	// Load the vertex array with data. position(x y z) | color(RGBa) | Texture
	...
	//unsigned n_inside = 0;
	//#pragma omp parallel for schedule(dynamic) reduction(+:n_inside) /* TODO */
	
	
	/*
	"Тестирование програмного обеспечения": Сэм Каннер
	«Быстрое тестирование": Robert Culbertson: 
	Lisa Crispin and Janet Gregory — “Agile Testing : A Practical Guide for Testers and Agile Teams”,“More Agile Testing”
...
bool ModelClass::LoadModel(char* filename)
{
	...
	// Open the model file.
	fin.open(filename);
	if (fin.fail())	// If it could not open the file then exit.
	{
		return false;
	}
