...
bool UserInterfaceClass::Initialize(D3DClass* Direct3D, int screenHeight, int screenWidth)
{
	...
	result = m_Font1->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), "../Engine/data/font/font01.txt", "../Engine/data/font/font01.tga", 32.0f, 3);
	if (!result)
	{
		return false;
	}
...
bool UserInterfaceClass::Render(D3DClass* Direct3D, ShaderManagerClass* ShaderManager, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
bool UserInterfaceClass::UpdateFpsString(ID3D11DeviceContext* deviceContext, int fps)
{
	...
	// If fps is 60 or above set the fps color to purple.
	if (fps >= 160)
	{
		red = 0.8f;
		green = 0.0f;
...
bool UserInterfaceClass::UpdateCpuString(ID3D11DeviceContext* deviceContext, int cpu)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = m_FpsString->UpdateSentence(deviceContext, m_Font1, finalString, 10, 70, 0.0f, 1.0f, 0.0f);
	if (!result)
	{
		return false;
	}
											   ...
											   float rotX, float rotY, float rotZ)
{
	...
	if (positionX != m_previousPosition[0])
	{
		...
		strcat_s(finalString, tempString);
		result = m_PositionStrings[0].UpdateSentence(deviceContext, m_Font1, finalString, 10, 110, 1.0f, 1.0f, 1.0f);
	...
	if (positionY != m_previousPosition[1])
	{
		...
		strcat_s(finalString, tempString);
		result = m_PositionStrings[1].UpdateSentence(deviceContext, m_Font1, finalString, 10, 130, 1.0f, 1.0f, 1.0f);
		if (!result) { return false; }
	}
	...
	if (positionZ != m_previousPosition[2])
	{
		...
		strcat_s(finalString, tempString);
		result = m_PositionStrings[2].UpdateSentence(deviceContext, m_Font1, finalString, 10, 150, 1.0f, 1.0f, 1.0f);
		if (!result) { return false; }
	}
	...
	if (rotationX != m_previousPosition[3])
	{
		...
		strcat_s(finalString, tempString);
		result = m_PositionStrings[3].UpdateSentence(deviceContext, m_Font1, finalString, 10, 190, 1.0f, 1.0f, 1.0f);
		if (!result) { return false; }
	}
	...
	if (rotationY != m_previousPosition[4])
	{
		...
		strcat_s(finalString, tempString);
		result = m_PositionStrings[4].UpdateSentence(deviceContext, m_Font1, finalString, 10, 210, 1.0f, 1.0f, 1.0f);
		if (!result) { return false; }
	}
	...
	if (rotationZ != m_previousPosition[5])
	{
		...
		strcat_s(finalString, tempString);
		result = m_PositionStrings[5].UpdateSentence(deviceContext, m_Font1, finalString, 10, 230, 1.0f, 1.0f, 1.0f);
		if (!result) { return false; }
	}
