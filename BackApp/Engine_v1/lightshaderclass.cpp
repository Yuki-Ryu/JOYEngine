...
LightShaderClass::LightShaderClass()
{
	...
	m_sampleState = 0;
	m_matrixBuffer = 0;
	m_cameraBuffer = 0;	// Initialize the new camera constant buffer to null in the class constructor.
	m_lightBuffer = 0;	// Set the light constant buffer to null in the class constructor.
}
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_INPUT_ELEMENT_DESC polygonLayout[3];	// The polygonLayout variable has been changed to have three elements instead of two.This is so that it can accommodate a normal vector in the layout.
	unsigned int numElements;
	D3D11_SAMPLER_DESC samplerDesc;
	D3D11_BUFFER_DESC matrixBufferDesc;
	D3D11_BUFFER_DESC cameraBufferDesc;
	D3D11_BUFFER_DESC lightBufferDesc;	// We also add a new description variable for the light constant buffer.
